...
def interval_censor(y, scale=0.5, option="random", **kwargs):
    if option == "random":
...
def execution(
    ...
    # [-] Generate data
    data = generate_data(sample_size=sample_size, scale=scale)
    n_eval = 50
    n_grid = 100
    n_partition = 5
...
rslt_df = pd.read_csv(
    "simulation_results_0.1_20241125-101420_random_censoring_1_chisquare.csv"
)
# %%
df_avg = (
